...
async function recordGeneratedAnswer(answer, correct) {
    ...
    window.cantoRecordAttempt?.({ type: selectedMode, wordId: generatedQuestion.id, prompt: generatedQuestion.word, answer, correctAnswer: wanted, isCorrect: ok });
    ...
    try {
        const response = await fetch("/api/activity", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ kind: "quiz_attempt", quizType: selectedMode, wordId: generatedQuestion.id, prompt: generatedQuestion.word, answer, correctAnswer: wanted, isCorrect: ok }) });
        document.getElementById("generatedSaveStatus").textContent = response.ok ? "本题记录已保存。" : "登录后可保存答题记录。";
    } catch (error) {
        document.getElementById("generatedSaveStatus").textContent = "本题暂未同步。";
    }
