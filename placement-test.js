(function () {
    ...
    function closeJyutpingChoices(item) {
        const candidates = [item.jyutping, toneVariant(item.jyutping, 1), soundVariant(item.jyutping, "initial"), soundVariant(item.jyutping, "final"), toneVariant(item.jyutping, 2)];
    ...
    async function markLevelComplete() {
        ...
        return Boolean(window.cantoCloud?.getSession?.());
    }
