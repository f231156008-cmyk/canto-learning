...
foreach ($item in $words) {
    ...
    $lines.Add("insert into public.words (legacy_id, theme_id, word, jyutping, difficulty, level, meaning_zh, pronunciation_note, audio_status, audio_review_note, audio_female, audio_male) select $($item.id), id, $(SqlText $item.word), $(SqlText $item.jyutping), $($item.difficulty), $(SqlText $item.level), $(SqlText $item.meaning), $(SqlText $item.pronunciationNote), $(SqlText $audioStatus), $(SqlText $item.audioReviewNote), $(SqlText $female), $(SqlText $male) from public.themes where name = $(SqlText $item.category) on conflict (legacy_id) do update set theme_id = excluded.theme_id, word = excluded.word, jyutping = excluded.jyutping, difficulty = excluded.difficulty, level = excluded.level, meaning_zh = excluded.meaning_zh, pronunciation_note = excluded.pronunciation_note, audio_status = excluded.audio_status, audio_review_note = excluded.audio_review_note, audio_female = excluded.audio_female, audio_male = excluded.audio_male, updated_at = now();")
    if (-not [string]::IsNullOrWhiteSpace([string]$item.example)) {
        $sentenceFemale = if ($item.sentenceAudio) { $item.sentenceAudio.female } else { $null }
        $sentenceMale = if ($item.sentenceAudio) { $item.sentenceAudio.male } else { $null }
        $lines.Add("insert into public.examples (word_id, sentence, translation_zh, audio_female, audio_male) select id, $(SqlText $item.example), $(SqlText $item.translation), $(SqlText $sentenceFemale), $(SqlText $sentenceMale) from public.words where legacy_id = $($item.id) and not exists (select 1 from public.examples where examples.word_id = words.id and examples.sentence = $(SqlText $item.example));")
    }
}
